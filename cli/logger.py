# Some code borrowed from InfraRed project.
...
def gitbrancher_excepthook(exc_type, exc_value, exc_traceback):
    """
    exception hook that sends GitBrancherException to log and other
    exceptions to stderr (default excepthook)
    """

    # sends full exception with trace to log
    if not isinstance(exc_value, exceptions.GitBrancherException):
...
sys.excepthook = gitbrancher_excepthook
